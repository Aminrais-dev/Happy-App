...
func (repo *eventData) SelectEventDetail(idEvent, userId int) (event.EventDetail, error) {
	...
	repo.db.Model(&JoinEvent{}).Select("count(join_events.id) as partisipasi").Where("event_id = ? AND status_payment = ? ", dataRes.ID, config.SUCCESS).Scan(&dataRes.Partisipasi)
